...
// Protect all admin routes - admin area only
...
router.post('/admin/sites/:id/edit', async (req, res) => {
  ...
  await pool.query(
    `
      ...
      WHERE id = ?
    `,
    [number, type, Number(lengthFt), description || '', active ? 1 : 0, req.params.id]
  );
  res.redirect('/admin/sites');
});

// Soft-delete / archive site (no hard delete)
router.post('/sites/:id/delete', requireRole('admin'), async (req, res) => {
  // Soft-delete / archive the site instead of hard delete to avoid orphaned reservations
...
router.get('/admin/walkin_reports', async (req, res) => {
  const [sites] = await pool.query('SELECT * FROM Site WHERE active = 1 ORDER BY number ASC');
    ...
    sites.map(async (site) => {
      const [[nextRes]] = await pool.query(
        `
          SELECT checkIn
          FROM Reservation
          WHERE siteId = ? AND status = 'CONFIRMED' AND checkIn > CURDATE()
...
// Availability report for a future date range
...
// Unpaid walk-ins (for admin follow-up)
...
// (Other admin user CRUD routes left as-is)
router.get('/admin/users', async (req, res) => {
  const [users] = await pool.query(
    'SELECT id, username, email, role, dod_affiliation, branch, rank_grade FROM users'
  );
...
router.post('/admin/users/new', async (req, res) => {
  try {
    ...
    await pool.query(
      'INSERT INTO users (username, email, first_name, last_name, password_hash, role, dod_affiliation, branch, rank_grade, num_adults, num_pets, pet_disclaimer_accepted, base_access_confirmed) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 0, 0, 0)',
      [
        username,
        email,
        'Admin',
